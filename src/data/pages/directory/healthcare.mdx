...
Every notification event in the Healthcare app should ideally include these four essential properties:

#### `event` - Event Identifier

	- Purpose: A stable, unique identifier used for grouping notification templates and routing logic.
	- Best practices:
		- Use kebab-case naming (e.g., appointment-booked, test-results-ready).
...
#### `recipientId` - Target Recipients
	...
	- Formats supported:
		- Individual user ID (e.g., a patient, doctor, or admin)
		- Topic-based (e.g., all staff in a department, all participants in an appointment)
...
#### `data` - Notification Context
	...
	- Should include:
		- Primary entity details (ID, type, status).
		- Actor information (who performed the action, if applicable).
		- Action metadata (what happened, when).
		- Relevant relationships (e.g., associated patient, doctor, department, or facility).
		- Additional context specific to the event (e.g., appointment time, test result summary, message content).
    
#### `cta` - Call-to-Action

	- Purpose: Provides the deep-link action that users can take directly from the notification.
...
```javascript
	...
	await novu.trigger("appointment.reminder", {
	  to: [{ subscriberId: "patient_user_123" }], 
	  payload: {
	    // Primary entity information (Appointment)
	    id: "appt_67890",
	    type: "Annual Checkup",
...
```json
	{
	  "event": "patient.testResultsReady",
	  "data": {
	    "testType": "Blood Work",
	    "resultSummary": "Results are available for review.",
	    "performedBy": { "id": "lab_tech_789", "name": "LabCorp" },
	    "patient": { "id": "patient_user_123", "name": "Alice Smith" },
	    "cta": {
	      "label": "View Results",
...
```json
	{
	  "event": "internal.newPatientAdmission",
	  "data": {
	    "patientName": "Bob Johnson",
	    "admissionTime": "2025-07-27T14:30:00Z",
	    "ward": "Emergency",
	    "admittingDoctor": { "id": "doctor_user_101", "name": "Dr. Sarah Lee" },
	    "cta": {
	      "label": "View Patient Details",
	      "url": "/app/admin/patient/bob_johnson_admission"
	    }
	  }
	}
```
...
```json
	{
	  "event": "facility.maintenanceAlert",
	  "data": {
...
```json
	{
	  "event": "patient.appointmentBooked",
	  "recipientId": "patient_user_123",
	  "payload": {
	    "appointmentId": "appt_A1B2C3",
	    "appointmentType": "Annual Checkup",
	    "appointmentDateTime": "2025-08-15T10:00:00Z",
	    "doctorName": "Dr. Sarah Lee",
	    "clinicLocation": "Main Clinic, Room 301",
...
```json
	{
	  "event": "patient.appointmentReminder",
	  "to": [{ "subscriberId": "patient_user_123" }],
	  "payload": {
	    "appointmentId": "appt_A1B2C3",
	    "appointmentType": "Annual Checkup",
...
```json
	{
	  ...
	  "payload": {
	    ...
	    "resultDate": "2025-07-26T14:00:00Z",
...
```json
	{
	  "event": "patient.messageReceived",
	  "recipientId": "patient_user_123",
	  "payload": {
	    "messageId": "msg_D4E5F6",
	    "senderName": "Dr. Mark Johnson",
...
```json
	{
	  "event": "doctor.newAppointment",
	  "recipientId": "doctor_user_456",
	  "payload": {
	    "appointmentId": "appt_A1B2C3",
	    "appointmentType": "Annual Checkup",
	    "appointmentDateTime": "2025-08-15T10:00:00Z",
	    "patientName": "Alice Smith",
	    "patientId": "patient_user_123",
	    "clinicLocation": "Main Clinic, Room 301",
	    "cta": {
...
```json
	{
	  "event": "doctor.patientMessage",
	  "recipientId": "doctor_user_456",
	  "payload": {
	    "messageId": "msg_G7H8I9",
	    "patientName": "Bob Johnson",
	    "patientId": "patient_user_789",
...
```json
	{
	  "event": "admin.staffLicenseExpiring",
	  "recipientId": "healthProvider_123_admin_userId",
	  "payload": {
	    "staffId": "staff_J1K2L3",
	    "staffName": "Nurse Carol",
...
```json
	{
	  "event": "staff.policyUpdate",
	  "to": [{ "type": "Topic", "topicKey": "healthProvider_123_staff" }],
	  "payload": {
	    "policyTitle": "New Data Privacy Policy (HIPAA Compliance)",
	    "policyId": "policy_M1N2O3",
...
```json   
	{
	  "event": "department.newImagingProtocol",
	  "to": [{ "type": "Topic", "topicKey": "healthProvider_123_cardiology" }],
	  "payload": {
	    "protocolName": "Advanced Cardiac MRI Protocol",
	    "protocolId": "protocol_P1Q2R3",
	    "implementationDate": "2025-08-10",
	    "responsibleDepartment": "Cardiology",
	    "cta": {
