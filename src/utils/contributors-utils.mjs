...
const fetchContributorsData = async () => {
  try {
    ...
    return contributorsWithPulls.sort(
...
const getContributorsWithAllAchievements = async (
  ...
  const contributorsWithAllAchievements = contributors
    .map((contributor) => {
      ...
      const additionalAchievements =
        contributorsWithAdditionalAchievements.find(
      ...
      return {
        ...contributor,
        achievements: [
          ...(additionalAchievements?.achievements || []),
          ...achievements,
        ],
      };
    })
  ...
  return contributorsWithAllAchievements.map(
...
const changeReadmeContent = async (
  readmeContent,
  contributorsWithAdditionalAchievements
) => {
  const heading = "## 🦸 Community Heroes\n\n";
  const data = await getContributorsWithAllAchievements(
    contributorsWithAdditionalAchievements
  );
