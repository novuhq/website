...
const Logos = () => (
  <section className="logos safe-paddings bg-black pb-[120px] pt-2 lg:pb-16 md:pb-14 sm:py-9">
    <div className="container">
      ...
      <ul className="mx-auto mt-12 flex w-full max-w-[1200px] flex-wrap justify-center gap-y-[42px] lg:max-w-[1000px] lg:gap-y-9 md:flex-wrap md:gap-x-12 md:gap-y-7 md:space-x-0 sm:mt-6 sm:gap-x-5 sm:gap-y-5">
        {LOGOS.map(({ src, alt, width, height }, index) => (
          ...
          >
            <img
              className={clsx(index < 5 ? 'h-12 lg:h-10 md:h-8 sm:h-6' : 'h-6', 'w-auto')}
              src={src}
              loading="eager"
              alt={alt}
              width={width}
