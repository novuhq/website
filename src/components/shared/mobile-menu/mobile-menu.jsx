...
const MobileMenu = ({ isOpen = false }) => {
  ...
  const handleOpenMenu = (label, content, variant) => {
    setOpenMenu((current) =>
      current.label === label ? defaultMenuContent : { label, content, variant }
    );
  };
  ...
  useEffect(() => {
    if (!isOpen) setOpenMenu(defaultMenuContent);
  }, [isOpen]);
    ...
    <LazyMotion features={domAnimation}>
      <AnimatePresence>
        {isOpen && (
          <m.div
            className={clsx(
              'safe-paddings pointer-events-none fixed inset-0 hidden w-full flex-col justify-between overflow-x-hidden overflow-y-scroll pt-16 font-inter md:flex',
            ...
            >
              <ul className="relative flex h-full w-full flex-col border-t border-t-[#1F1F1F] px-8 sm:px-5">
                {MENUS.header.map(({ text, content, to, target, variant }) => {
                  const isExpanded = openMenu.label === text;

                  return (
                    <li className="border-b border-b-[#1F1F1F] last:border-b-0" key={text}>
                      <Link
                        className="flex h-[54px] w-full items-center justify-between text-left text-lg font-medium leading-none text-white sm:h-[45px] sm:text-base"
                        tag={content ? 'button' : null}
                        to={to}
                        target={target}
                        type={content ? 'button' : null}
                        aria-expanded={content ? isExpanded : null}
                        onClick={content ? () => handleOpenMenu(text, content, variant) : undefined}
                      >
                        {text}
                        {content && (
                          <ChevronIcon
                      ...
                      <InnerMenu openMenu={openMenu} label={text} />
                    </li>
                  );
                })}
              </ul>
