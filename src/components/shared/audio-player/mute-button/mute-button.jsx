...
const MuteButton = ({ player }) => (
  <button
    type="button"
    className="hover:bg-slate-100 focus:ring-slate-400 group relative order-none rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2"
    onClick={() => player.toggleMute()}
  >
    <div className="absolute -inset-4 hidden md:block" />
    <span className="sr-only">{player.muted ? 'Unmute' : 'Mute'}</span>
    <svg
      className="h-6 w-6 fill-gray-8 stroke-gray-8 group-hover:fill-white group-hover:stroke-white"
      strokeWidth="2"
      strokeLinecap="round"
